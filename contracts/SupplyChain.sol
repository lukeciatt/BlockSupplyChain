...
contract supplyChain {
    ...
    // Basic structure of products, this change depending on the company, of course.
    struct product {
        string modelNumber;
        string partNumber;
        string serialNumber;
    ...
    // We create a list linking the products ids (uint32) with the structures of the products itself (product) and we define them as public and we call this list 'products'
    ...
    // here we link participants to products based on their ids
    struct ownership {
        uint32 productId;
        uint32 ownerId;
        uint32 trxTimeStamp;
    ...
    mapping(uint32 => ownership) public ownerships; // ownerships by ownership ID (owner_id), answer the question 'how many products an owner have'
    mapping(uint32 => uint32[]) public productTrack;  // ownerships by Product ID (product_id) / Movement track for a product, anwer the questions 'where have you been?'
